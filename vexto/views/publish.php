...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $can_publish) {
    ...
    try {
        ...
        if ($property_id) {
            if ($isAdmin) {
                $stmt = $pdo->prepare("UPDATE properties SET titulo = ?, descripcion = ?, precio = ?, tipo_operacion = ?, tipo_propiedad = ?, ubicacion = ?, latitud = ?, longitud = ?, habitaciones = ?, banos = ?, area_m2 = ?, imagen_url = ?, imagen_tipo = ? WHERE id = ?");
                $stmt->execute([$titulo, $descripcion, $precio, $tipo_operacion, $tipo_propiedad, $ubicacion, $lat, $lng, $habitaciones, $banos, $area_m2, $imagen_url, $imagen_tipo, $property_id]);
            } else {
